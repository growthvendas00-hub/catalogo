...
export async function getPublicOrderByToken(token: string): Promise<Order | null> {
  if (!hasSupabaseSecretEnv) return null;
  const supabase = createSupabaseAdminClient();
  const { data, error } = await supabase.from("orders").select(orderSelect).eq("public_token", token).maybeSingle();
  if (error) throw new Error(`Não foi possível carregar o pedido: ${error.message}`);
  return data ? mapOrder(data as DbOrder) : null;
}
