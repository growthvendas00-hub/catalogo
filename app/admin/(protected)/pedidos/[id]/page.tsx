...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="grid gap-1 border-b fine-rule py-3 sm:grid-cols-[11rem_1fr] sm:gap-5"><dt className="eyebrow text-black/45">{label}</dt><dd className="text-sm leading-relaxed break-words">{children}</dd></div>;
}
  ...
  return (
    <main id="conteudo" className="p-4 sm:p-7 lg:p-10">
      ...
      <header className="mt-6 border-b fine-rule pb-7">
        <p className="eyebrow text-black/45">Pedido de {formatDateTime(order.createdAt)}</p>
      ...
      <div className="grid gap-10 py-8 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,.55fr)]">
        ...
        <aside><OrderStatusForm id={order.id} status={order.fulfillmentStatus} notes={order.adminNotes} /><a className="button-secondary mt-5 w-full" href={`/pedido/${order.publicToken}`} target="_blank" rel="noreferrer"><ExternalLink size={15} /> Ver acompanhamento</a></aside>
      </div>
    </main>
  );
}
